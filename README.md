...
### The Easy Way — Admin Panel + Cloud Sync

After the one-time Supabase setup described in `SUPABASE-SETUP.md`:

1. Open `https://YOUR-USERNAME.github.io/admin.html`
2. Sign in with your admin account
3. Use the sidebar to navigate to the section you want to update
4. Make your changes using the existing forms
5. Click **Save**
6. The change is saved to Supabase automatically and the public website receives the update in real time
7. Click **Preview Site** to see the result

The browser `localStorage` is still used as a fast local cache, but Supabase is now the permanent source of truth.

> ⚠️ **IMPORTANT:** Never put a Supabase `service_role`/secret key in the website. Only use the public anon/publishable key in `supabase-config.js`.
